...
var utils = {
  ...
  downloadUrl: function(item, callback) {
    ...
    var hash = crypto.createHash('md5').update(item.url).digest("hex");
    var filepath = DOWNLOADS_DIR + hash + extension;
  ...
  cleanDownloads: function(max_downloads, callback) {
    ...
    files.sort(function(a, b) {
                   return fs.statSync(DOWNLOADS_DIR + a).mtime.getTime() -
                          fs.statSync(DOWNLOADS_DIR + b).mtime.getTime();
               });

    if(files.length > max_downloads) {
      for(var i= max_downloads; i < files.length; i++) {
        console.log("Removing file "+DOWNLOADS_DIR+files[i]);
