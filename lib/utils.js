...
var utils = {
  ...
  downloadUrl: function(url, filepath, callback) {
    ...
    utils.cache[filepath] = utils.cache[filepath] || {};

    // If file has already been previously downloaded
    if(fs.existsSync(filepath)) {
      var filesize = fs.statSync(filepath).size;
      if(filesize == utils.cache[filepath].filesize) {
        console.log("Getting file " + filepath + " from cache");
        return callback(null, '/'+filepath);
      }
      else {
        console.log("\""+filepath+"\" size ("+filesize+") does not match original size ("+utils.cache[filepath].filesize+")");
      }
    }
    ...
    stream.on('response', function(res) {
      ...
      utils.cache[filepath].filesize = parseInt(res.headers['content-length'],10); 
    });
  ...
  cleanDownloads: function(max_downloads, callback) {
    ...
    if(files.length > max_downloads) {
      for(var i= max_downloads; i < files.length; i++) {
        ...
        delete utils.cache[DOWNLOADS_DIR+files[i]];
